...
function spawner(command, args, options) {
    return child_process_1.spawn(command, args, options).on('exit', function (code) {
        if (code !== 0) {
            process.exit(code);
        }
    });
...
function test(args) {
    spawner('react-app-rewired', __spreadArrays(['test', '--env=jest-environment-jsdom-fourteen'], args), {
        shell: true,
        stdio: 'inherit',
        env: env,
    });
...
function build(args) {
    spawner('react-app-rewired', __spreadArrays(['build'], args), {
        shell: true,
        stdio: 'inherit',
        env: env,
    });
...
function buildLib(args) {
    ...
    var webpack;
    if (args[0] === '--verbose') {
        webpack = spawner('webpack', ['--config', path_1.resolve(__dirname, '../webpack.config.js')], {
            shell: true,
            stdio: 'inherit',
            env: env,
        });
    ...
    (_a = webpack.stdout) === null || _a === void 0 ? void 0 : _a.on('data', function (data) {
        if (data.toString().includes('ERROR')) {
            console.log(chalk_1.default.red('An error occurred during the build!'));
